...
export function PropTab({ projectId }: PropTabProps) {
  ...
  const fetchAssets = useCallback(async () => {
    setLoading(true);
    try {
      const data = await propApi.list(projectId);
      setAssets(data);
    } catch {
  ...
  const handleCreateAsset = async (data: CreateAssetRequest) => {
    try {
      ...
      await propApi.create(projectId, { ...data, orgId });
  ...
  const handleUpdateAsset = async (id: string, data: UpdateAssetRequest) => {
    try {
      ...
      await propApi.update(id, updateData);
      if (coverImageId) {
