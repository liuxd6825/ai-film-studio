  ...
  return (
    <div className="relative">
      <input
        ...
        className="w-full h-6 px-2 text-xs border border-blue-500 rounded bg-white focus:outline-none focus:ring-1 focus:ring-blue-500"
...
export function MainContent() {
  ...
  if (!selectedStoryPage) {
    return (
      <div className="flex-1 flex items-center justify-center text-zinc-500 flex-col gap-4">
        <Clapperboard size={48} className="text-zinc-300" />
        <p>请在左侧选择一个故事页</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-zinc-50/50">
      <header className="bg-white border-b flex flex-col shrink-0">
          ...
          <div>
            <div className="flex items-baseline gap-3">
              ...
              <p className="text-sm text-zinc-500 max-w-3xl">
                {selectedStoryPage.desc}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-zinc-100 p-0.5 rounded-md border border-zinc-200 mr-2">
              <button
                className={`p-1.5 rounded flex items-center justify-center transition-colors ${showShotNav ? "bg-white shadow-sm text-zinc-900" : "text-zinc-500 hover:text-zinc-700"}`}
              ...
              <button
                className={`p-1.5 rounded flex items-center justify-center transition-colors ${viewMode === "edit" ? "bg-white shadow-sm text-zinc-900" : "text-zinc-500 hover:text-zinc-700"}`}
                onClick={() => setViewMode("edit")}
              >
                <List size={14} />
              </button>
              <button
                className={`p-1.5 rounded flex items-center justify-center transition-colors ${viewMode === "browse" ? "bg-white shadow-sm text-zinc-900" : "text-zinc-500 hover:text-zinc-700"}`}
            ...
            <div className="flex items-center gap-3 text-xs text-zinc-600 bg-zinc-50 px-3 py-1.5 rounded-md border">
        ...
        {showShotNav && pageShots.length > 0 && (
          <div className="px-4 py-2 bg-zinc-50/80 border-t border-zinc-100 flex items-start sm:items-center gap-2">
            <div className="flex-1 flex items-center gap-1.5 flex-wrap">
              {currentNavShots.map((shot) => (
                <button
                  key={shot.id}
                  onClick={() => handleNavigate(selectedStoryPage.id, shot.id)}
                  className="px-2.5 py-1.5 rounded-md text-xs text-zinc-600 hover:bg-zinc-200 hover:text-zinc-900 transition-colors shrink-0 flex items-center gap-1.5"
                >
            ...
            {totalNavPages > 1 && (
              <div className="flex items-center gap-1 shrink-0 ml-auto pl-2 border-l border-zinc-200 mt-0.5 sm:mt-0">
                <button
                  className="p-1.5 rounded hover:bg-zinc-200 text-zinc-500 disabled:opacity-50"
                  disabled={shotNavPage === 0}
                  onClick={() => setShotNavPage((p) => Math.max(0, p - 1))}
                >
                  <ChevronLeft size={14} />
                </button>
                <span className="text-[10px] text-zinc-500 font-medium min-w-[30px] text-center">
          ...
          <div className="space-y-2 pb-12">
              ...
              pageShots.map((shot) => {
                ...
                return (
                  <div
                    key={shot.id}
                    id={`shot-${shot.id}`}
                    className="bg-white rounded-lg border shadow-sm scroll-mt-4 flex items-stretch"
                    onDragOver={(e) => handleDragOver(e, `shot-${shot.id}`)}
                    onDragLeave={handleDragLeave}
                    onDrop={(e) => handleDropOnShot(e, shot.id)}
                  >
                    <div className="w-1/4 min-w-0 border-r border-zinc-200 bg-zinc-50/50 flex flex-col">
                      <div className="px-3 py-2 border-b border-zinc-200 bg-zinc-50/90 flex items-center justify-between shrink-0">
                        <div className="flex items-center gap-2">
                          <input
                            type="checkbox"
                      ...
                      <div className="flex flex-1">
                        <div className="w-2/5 p-2 border-r border-zinc-200 flex flex-col flex-1 min-h-0">
                          ...
                          editingShotField?.field === "content" ? (
                            <textarea
                              className="w-full flex-1 px-2 py-1 text-[10px] border border-blue-500 rounded bg-white focus:outline-none resize-none"
                            ...
                            <div
                              className="flex-1 min-h-0 p-2 text-[10px] text-zinc-700 bg-zinc-100/50 rounded border border-transparent cursor-pointer hover:border-zinc-300 overflow-y-auto"
                              onClick={() =>
                                setEditingShotField({
                                  shotId: shot.id,
                                  field: "content",
                        ...
                        <div className="w-1/5 p-2 border-r border-zinc-200 space-y-1">
                          {editingShotField?.shotId === shot.id &&
                          editingShotField?.field === "sceneType" ? (
                            ...
                            >
                              <span className="text-zinc-400">场景:</span>
                          ...
                          editingShotField?.field === "weather" ? (
                            ...
                            >
                              <span className="text-zinc-400">天气:</span>
                          ...
                          editingShotField?.field === "cameraAngleH" ? (
                            ...
                            >
                              <span className="text-zinc-400">水平:</span>
            ...
            {isShotsLoading ? (
              <div className="text-center py-20 bg-white rounded-xl border border-dashed">
                <div className="animate-spin w-8 h-8 border-2 border-zinc-300 border-t-zinc-600 rounded-full mx-auto mb-4" />
                <h3 className="text-lg font-medium text-zinc-900">加载中...</h3>
                <p className="text-sm text-zinc-500 mt-1">正在加载镜头数据</p>
              </div>
            ) : pageShots.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-xl border border-dashed">
                <Video size={48} className="mx-auto text-zinc-300 mb-4" />
                <h3 className="text-lg font-medium text-zinc-900">暂无镜头</h3>
