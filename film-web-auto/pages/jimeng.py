...
class JimengPage(BasePage):
    ...
    async def wait_for_page_ready(self):
        ...
        try:
            await self.page.wait_for_selector("div[class^='entry-'] div[class^='toolbar-']", timeout=15000)
    ...
    async def _get_current_mode(self) -> str:
        ...
        # mode_select = self.page.locator(selector).first
        # mode_select = self.page.locator("div[class^='entry-'] div[class^='main-content-'] div[class^='toolbar-settings-content-'] div[role='combobox']").first
    ...
    async def _select_dropdown_option(self, mode_text: str):
        ...
        await self.page.get_by_role("combobox").filter(has_text="Agent 模式").first.click(force=True)
        # await self.page.locator("div[class^='entry-'] div[class^='main-content-'] div[class^='toolbar-settings-content-'] div[role='combobox']").first.click(force=True)
        await self.page.wait_for_timeout(1000)
    ...
    async def select_model(self, model: str):
        ...
        all_selects = await self.page.locator(
            "div[class^='entry-'] div[class*='toolbar-settings-content-'] .lv-select").all()
    ...
    async def select_reference_type(self, ref_type: str):
        ...
        feature_select = self.page.locator("div[class^='feature-select-']")
    ...
    async def input_prompt(self, prompt: str) -> str:
        id = uuid.uuid4().hex
        editor = self.page.locator("div[class^='prompt-editor-'] .tiptap.ProseMirror").first
    ...
    async def open_settings_popup(self):
        await self._close_any_open_dropdown()
        settings_btn = self.page.locator("div[class^='entry-'] div[class*='toolbar-settings-content-'] button")
    ...
    async def select_aspect_ratio(self, ratio: str):
        ...
        all_radios = await self.page.locator(
            "div.lv-radio-group:not([class*='resolution-radio-group-']):not([class*=' resolution-radio-group-']) > label").all()
    ...
    async def select_resolution(self, resolution: str):
        ...
        all_radios = await self.page.locator(
            "div[class*='resolution-radio-group-']>label.lv-radio,div[class*=' resolution-radio-group-']>label.lv-radio").all()
        for radio in all_radios:
            text = await radio.text_content()
            if text and normalized in text.lower():
                await radio.click(force=True)
    ...
    async def submit(self):
        submit_btn = self.page.locator("div[class^='entry-'] div[class^='toolbar-actions-'] button.lv-btn-primary")
        if await submit_btn.count() > 0:
            disabled = await submit_btn.first.get_attribute("disabled")
            if disabled is None:
                await submit_btn.first.click(force=True)
            else:
                submit_btn = self.page.locator("div[class^='toolbar-actions-'] button").last
    ...
    async def wait_for_generation_complete(self, timeout: int = 60000) -> dict:
        ...
        download_urls = []
        file_names = []
        div_list = self.page.locator("div[class^='virtua-list-']>div>div>div")
        if div_list.count() == 0:
            return {
                "status": "completed",
                "message": "生成成功",
        ...
        target_div = div_list.nth(div_list.count() - 2)
        while True:
            if target_div:
                text = await target_div.locator("div[class^='record-box-wrapper-']").text_content() or ""
    ...
    async def wait_for_generation_complete_v2(self, timeout: int = 10000) -> dict:
        ...
        target_div = div_list.nth(div_list_count - 2)

        while elapsed < 10:
            if target_div:
                div_box = target_div.locator("div[class^='record-box-wrapper-']")
                ...
                text = await div_box.text_content() or ""
